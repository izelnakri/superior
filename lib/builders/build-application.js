...
import convertTSModuletoAMD from '../transpilers/convert-ts-module-to-amd';
...
export default function(buildConfig={
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const excludeTestFiles = (item) => {
      return !item.path.endsWith('-test.js') || !item.path.endsWith('-test.ts');
    };
  ...
  return new Promise((resolve) => {
    return fs.readFile(fileName).then((fileContents) => {
      if (fileName.endsWith('.js')) {
        ...
        return resolve(convertTSModuletoAMD(fileContents, { moduleName: getModuleName(fileName, appName) }));
